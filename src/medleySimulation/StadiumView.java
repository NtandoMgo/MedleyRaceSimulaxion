...
public class StadiumView extends JPanel implements Runnable {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i <= endPool; i++) // rows
			g.drawLine(wIncr * xBorder, (i + yBorder) * hIncr, (maxX + xBorder) * wIncr, (i + yBorder) * hIncr); // -
																													// leave
																													// space
																													// at
																													// sides
		...
		// Draw lane labels
		g.setColor(Color.BLACK); // Color for labels
		g.setFont(new Font("Arial", Font.BOLD, 12)); // Font style and size
		for (int laneNumber = 1; laneNumber <= 10; laneNumber++) {
			int xPosition = (laneNumber - 1) * 5 + xBorder + 1; // Calculate x position of the label
			int yPosition = (endPool + yBorder) * hIncr + 20; // Position below the lanes
			g.drawString(String.valueOf(laneNumber), xPosition * wIncr + (wIncr / 2) - 10, yPosition); // Draw label
		}
